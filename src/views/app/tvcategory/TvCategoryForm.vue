...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await TvCategoryApi.getTvCategory(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as TvCategoryVO
    if (formType.value === 'create') {
      await TvCategoryApi.createTvCategory(data)
      message.success(t('common.createSuccess'))
    } else {
      await TvCategoryApi.updateTvCategory(data)
