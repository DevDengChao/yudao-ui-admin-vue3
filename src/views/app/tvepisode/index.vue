<template>
  <ContentWrap>
    ...
    <el-form
      ...
      label-width="68px"
    >
      <el-form-item label="剧集 id" prop="id">
        <el-input
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      <el-table-column label="剧集 id" align="center" prop="id" />
      <el-table-column
        label="创建时间"
        align="center"
        prop="createTime"
      ...
      <el-table-column label="操作" align="center">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['app:tv-episode:update']"
          >
            编辑
          </el-button>
          <el-button
            ...
            v-hasPermi="['app:tv-episode:delete']"
          >
            删除
          </el-button>
        </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <TvEpisodeForm ref="formRef" @success="getList" />
</template>
...
const list = ref<TvEpisodeVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await TvEpisodeApi.getTvEpisodePage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await TvEpisodeApi.deleteTvEpisode(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await TvEpisodeApi.exportTvEpisode(queryParams)
    download.excel(data, '短剧剧集.xls')
  } catch {
  } finally {
